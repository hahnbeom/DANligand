...
def read_options():
    ...
    parser.add_argument("--tag",default="run1",help="temporary prefix to feature npz")
...
def featurize_if_missing(option):
    ...
    #else:
    featurize.main(option.tag,verbose=True,decoytypes=[''], 
                   inputpath = './',
                   outpath = './',
                   store_npz=True,
                   same_answer=False)
...
def main(option):
    ...
    with torch.no_grad(): # without tracking gradients
        val_set = Dataset([option.tag], root_dir=option.inputpath)
