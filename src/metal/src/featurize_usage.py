...
def grid_from_xyz(xyzs,xyz_lig,gridsize,
    ...
    for i,grid in enumerate(grids):
        print("HETATM %4d  CA  CA  X%4d    %8.3f%8.3f%8.3f"%(i,i,grid[0],grid[1],grid[2]))
...
def main(pdb,outprefix,
    ...
    npz = "%s/%s.prop.npz"%(outpath,outprefix)
    featurize_target_properties(pdb,npz,out)

    # featurize grid points
    _, reschains, xyz, atms = myutils.read_pdb(pdb)
    ...
    tags = []
    for i,grid in enumerate(grids):
        metalxyz.append(grid)
        tags.append(outprefix+'.%d'%i)
        
    npz = "%s/%s.lig.npz"%(outpath,outprefix)
    np.savez(npz,
             xyz=metalxyz,
             name=tags)
