...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        samples = np.load(self.datadir+pname+'.lig.npz',allow_pickle=True) #metal type & crd only
        ...
        xyz_metal   = samples['xyz'][pindex]

        metalidx = -1
        if 'metalidx' in samples:
            metalidx    = samples['metalidx'][pindex] #integer
        ...
        # randomize metal coordinate
        dxyz = np.zeros(3)
        if self.randomize_lig > 1e-3:
            dxyz = 2.0*self.randomize_lig*(0.5 - np.random.rand(3))

        # orient around xyz_metal + delta
        xyz = xyz - xyz_metal[None,:] - dxyz[None,:]
        ...
        try:
            ...
            #print("!!",pname,len(xyz),G_atm.number_of_nodes(),G_atm.number_of_edges())
            rsds_ord = r2a[idx_ord]
            G_res,r2amap = None,None
            #G_res, r2amap = self.make_res_graph(xyz, center_xyz, [aas1hot,sasa],
            #                                    repsatm_idx, rsds_ord)
            #print("Size:", info['sname'], len(xyz), len(repsatm_idx), G_atm.number_of_nodes()) #, G_res.number_of_nodes())
...
def collate(samples):
    ...
    B = len(samples)
    binfo = {'pname':[],'sname':[],'metalidx':[],'dxyz':[]}
    ...
    if True:
        ...
        for k in range(B):
            ...
            binfo['metalidx'].append(info[k]['metalidx'])
        ...
        binfo['r2a'] = torch.tensor(0).float()
        binfo['metalidx'] = torch.tensor(binfo['metalidx']).float()
