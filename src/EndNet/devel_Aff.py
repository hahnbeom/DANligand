...
from src.myutils import count_parameters, to_cuda, calc_AUC, affinity2weight
from src.dataset_v4 import collate, DataSet
import src.Loss as Loss

from args_v5 import args_Dkey as args
args.affinityf = 'data/affinity.npz'
args.modelname = "Aff"
...
def load_params(rank):
    ...
    ## loss
    train_loss_empty={"total":[],"BCEc":[],"BCEg":[],"BCEr":[],"contrast":[],"reg":[], "struct":[], "mae":[], "spread":[], "Screen":[], "ScreenC":[], "ScreenR":[]}
    valid_loss_empty={"total":[],"BCEc":[],"BCEg":[],"BCEr":[],"contrast":[],"struct":[], "mae":[], "spread":[], "Screen":[], "ScreenC":[], "ScreenR":[]}
...
def train_model(rank,world_size,dumm):
    ...
    ## data loader
    train_loader = load_data(args.datasetf[0], world_size, rank)
    valid_loader = load_data(args.datasetf[1], world_size, rank)
...
def train_one_epoch(model,optimizer,loader,rank,epoch,is_train):
    temp_loss={"total":[], "BCEc":[], "BCEg":[], "BCEr":[], "contrast":[], "reg":[], "struct":[], "mae":[], "spread":[], "Screen":[], "ScreenC":[], "ScreenR":[]}
    b_count,e_count=0,0
    accum=1
    device=torch.device("cuda:%d"%rank if (torch.cuda.is_available()) else "cpu")
        ...
        with torch.cuda.amp.autocast(enabled=False):
            with model.no_sync(): #should be commented if
                ...
                if Glig != None:
                    ...
                    afflabel = info['aff'].to(device)
                else:
                    keyxyz, keyidx, nK, blabel, afflabel = None, None, None, None, None
                ...
                if Yrec_s != None and grid.shape[1] == z.shape[1]:
                    try:
                        ...
                        affweight = affinity2weight(afflabel)
                        lossScreen = Loss.ScreeningLossW( aff[0], blabel, weight=affweight ) #shift by 6.0 (pKd 6 -> 0.5)
                        #lossScreen = Loss.AffinityLoss( aff[0], blabel ) # train to predict affinity instead
                        ...
                        #print("BCE vs Ranking:", Pbind, float(lossScreen), float(lossScreenR))
                ...
                loss = args.wGrid*(lossGc + lossGg + lossGr + lossGcontrast + \
                      ...
                      + args.wTR*(lossTs + lossTr) \
                      + args.w_screen*lossScreen + args.w_screen_ranking*lossScreenR + args.w_screen_contrast*lossScreenC
                
                #print(float(lossTs), float(lossTr), float(lossTs), float(lossScreen), float(lossScreenR), float(lossScreenC))
            ...
            if is_train and (b_count+1)%accum == 0:
                ...
                optimizer.step()    
                optimizer.zero_grad()
                print("Rank %d TRAIN Epoch(%s): [%2d/%2d], Batch: [%2d/%2d], loss: %8.3f (M/R/C/S(mae)/Sp/Scr: %6.1f/%6.1f/%6.3f/%6.1f(%5.2f)/%6.1f/%4.2f/%4.2f), %s:%s"
                      %(rank,args.modelname, epoch, args.max_epoch, b_count, len(loader),
                        ...
                        #np.sum(temp_loss["ScreenC"][-1*accum:]),
                        pnames[0], gridinfo
                      ), ' '.join(Pbind), ','.join(info['ligands'][0]))
                
            elif (b_count+1)%accum == 0: # valid
                print("Rank %d VALID Epoch(%s): [%2d/%2d], Batch: [%2d/%2d], loss: %8.3f (M/R/C/S(mae)/Sp/Scr: %6.1f/%6.1f/%6.3f/%6.1f(%5.2f)/%6.1f/%4.2f/%4.2f), %s:%s"
                      %(rank,args.modelname, epoch, args.max_epoch, b_count, len(loader),
                        np.sum(temp_loss["total"][-1*accum:]),
                        np.sum(temp_loss["BCEc"][-1*accum:]),
                        np.sum(temp_loss["BCEr"][-1*accum:]),
