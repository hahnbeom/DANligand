...
def load_params(rank):
    ...
    ## loss
    train_loss_empty={"total":[],"BCEc":[],"BCEg":[],"BCEr":[],"contrast":[],"reg":[], "struct":[], "mae":[], "spread":[], "Screen":[], "ScreenC":[]}
    valid_loss_empty={"total":[],"BCEc":[],"BCEg":[],"BCEr":[],"contrast":[],"struct":[], "mae":[], "spread":[], "Screen":[], "ScreenC":[]}
...
def train_one_epoch(model,optimizer,loader,rank,epoch,is_train):
    temp_loss={"total":[], "BCEc":[], "BCEg":[], "BCEr":[], "contrast":[], "reg":[], "struct":[], "mae":[], "spread":[], "Screen":[], "ScreenC":[]}
    b_count,e_count=0,0
    accum=1
    device=torch.device("cuda:%d"%rank if (torch.cuda.is_available()) else "cpu")
    ...
    for i, inputs in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            with model.no_sync(): #should be commented if
                ...
                lossScreen, lossScreenC = torch.tensor(0.0).to(device), torch.tensor(0.0).to(device)
                ...
                loss = args.wGrid*(lossGc + lossGg + lossGr + lossGcontrast + \
                      ...
                      + args.w_screen*( lossScreen + args.w_screen_contrast*lossScreenC )
            ...
            if is_train and (b_count+1)%accum == 0:
                ...
                optimizer.step()    
                optimizer.zero_grad()
                print("Rank %d TRAIN Epoch(%s): [%2d/%2d], Batch: [%2d/%2d], loss: %8.3f (M/R/C/S(mae)/Sp/Scr: %6.1f/%6.1f/%6.3f/%6.1f(%5.2f)/%6.1f/%4.2f/%4.2f), %s"
                ...
                print("Rank %d BALID Epoch(%s): [%2d/%2d], Batch: [%2d/%2d], loss: %8.3f (M/R/C/S(mae)/Sp/Scr: %6.1f/%6.1f/%6.3f/%6.1f(%5.2f)/%6.1f/%4.2f/%4.2f), %s"
                      %(rank,args.modelname, epoch, args.max_epoch, b_count, len(loader),
                        np.sum(temp_loss["total"][-1*accum:]),
                        np.sum(temp_loss["BCEc"][-1*accum:]),
                        np.sum(temp_loss["BCEr"][-1*accum:]),
