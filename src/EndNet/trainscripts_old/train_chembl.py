...
def load_params(rank):
    ...
    if os.path.exists("models/%s/classifier.pkl"%args.modelname):
        if not silent: print("Loading a checkpoint")
        checkpoint = torch.load(join("models", args.modelname, "classifier.pkl"),map_location=device)
    ...
    #       "class" in name: #".wr" in name or '.wl' in name:
    #        if rank == 0:
    #            print("reweight", name)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            with model.no_sync(): #should be commented if
                ...
                Yrec_s, z, MotifP, aff = model(Grec, 
                                               ...
                                               drop_out=is_train,
                                               trim_receptor_embedding=args.trim_receptor_embedding)
