...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.nsamples = max(1,len(self.npzs)*nsamples_per_p)
    ...
    def __getitem__(self, index):
        ...
        fname = self.datadir+self.npzs[ip] #npz looks like GridNet.ligand/[npz] 
        parentpath = '/'.join(fname.split('/')[:-1])+'/' # includes all absolute path

        is_ligand = ('ligand' in fname)
        if self.labeled:
            pname = self.npzs[ip].split('/')[-1].split('.')[0]
            if is_ligand:
                gridchain = None
                info['pname'] = pname

            else:
                gridchain = self.npzs[ip].split('/')[-1].split('.')[1]
                gridname = self.npzs[ip].split('/')[-1].split('.')[2]
                info['pname'] = pname+'.'+gridchain+'.'+gridname
        ...
        ## 2. read ligand info if exist
        Glig,labelxyz,keyidx = False,False,False
        if is_ligand:
            mol2 = parentpath+pname+'.ligand.mol2' ####### should change directory info after running featurize_lig###3
            ...
            keyidx = identify_keyidx(pname, Glig, atms, parentpath, self.K)
            if not keyidx:
            ...
            keyidx = keyidx_nat[:self.K] # hard-coded to use first 4 whatever it is
            keyxyz = xyzlig_nat[keyidx]
            
        else:
            #may this cause issue?
            keyidx = [0,0,0,0] # placeholder
            keyxyz = np.zeros((4,3)) # placeholder
            origin = torch.tensor(np.mean(grids,axis=0)) # orient around grid center
            
        ## 3. common route -- read receptor info
        prop = np.load(parentpath+pname+".prop.npz")
        xyz         = prop['xyz_rec']
        ...
        if isinstance(G_atm,int):
            print(" - Skip this (name/grid/node): ", info['pname'], is_ligand, G_atm, "%.1f sec"%(t1-t0))
            return
        elif self.labeled:
        ...
        return G_atm, Glig, keyxyz, keyidx, info 
...
def collate(samples):
    ...
    for s in valid:
        Grec.append(s[0])
        label.append(torch.tensor(s[3])) #motif category
        info.append(s[4])

    Grec = dgl.batch(Grec)
    node_feature = {"0": Grec.ndata["attr"][:,:,None].float(), 'x': Grec.ndata['x'].float()}
    edge_feature = {"0": Grec.edata["attr"][:,:,None].float()}
    ...
    if is_ligand:
        ...
        Glig = None
        labelxyz = torch.tensor(0.0)

    # below contains l0 features only
    return Grec, Glig, labelxyz, label, info, len(samples), node_feature, edge_feature
