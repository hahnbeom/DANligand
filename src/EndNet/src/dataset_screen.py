...
class DataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index): #N: maximum nodes in batch
        ...
        if ligands == 'batch':
            ...
            mol2type = 'batch'
            mol2f = self.datapath+'/%s.ligand.mol2'%pname
            ligands = myutils.read_mol2_batch(mol2f,tags_only=True)
        ...
        ligands = [target]+list(np.random.choice(ligands[1:],self.max_subset-1,replace=False))
        ...
        try:
            ...
            if is_ligand:
                ...
                blabel_list, Glig_list, keyidx_list, atms_list, Gnat, keyxyz, origin = self.read_ligands(target, ligands, keyatomf, mol2path=mol2f, mol2type=mol2type )
    ...
    def read_ligands(self, target, ligands, keyatomf, mol2path, mol2type='single'):
        if mol2type == 'batch':
            blabel_list, Glig_list, keyidx_list, atms_list, Gnat, keyxyz = read_by_batch_mol2(mol2path, keyatomf, -1)
            origin = []

        else:
            blabel_list, Glig_list, keyidx_list, atms_list, Gnat, keyxyz, origin = self.read_by_single_mol2(target, ligands, keyatomf, mol2path=mol2path)
    ...
    def read_by_single_mol2(self, target, ligands, keyatomf, mol2path):
        ...
        for lig in ligands:
            ...
            _Gnat,Glig,atms = ligand_graph_from_mol2(mol2,
    ...
    def read_by_batch_mol2(self, mol2, keyatomf, K, tags=[], randrot=False, random_sample=-1):
        Glig_list = [] 
        blabel_list = []
        keyidx_list = []
        atms_list = []
        ...
        return blabel_list, Glig_list, keyidx_list, atms_list, Gnat, keyxyz_list, tags_processed
