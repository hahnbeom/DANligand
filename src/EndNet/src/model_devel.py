...
class XformModule( nn.Module ):
    ...
    def forward( self, V, Q, z, z_mask, dim ): # V == key & value
        ...
        if dim == 1:
            Va = torch.einsum('bikc,bkc->bic', z, Qa) # B x N x c
        elif dim == 2:
            Va = torch.einsum('bikc,bic->bkc', z, Qa) # B x K x c
...
class EndtoEndModel(nn.Module):
    ...
    def forward(self, Grec, Glig, keyidx, grididx,
        ...
        for i,idx in enumerate(keyidx):
            key_idx[i,:idx.shape[0],:idx.shape[1]] = idx #1hot
            lig_to_key_mask[i,:idx.shape[0],:idx.shape[1]] = 1.0
            #print(i,key_idx[i], lig_to_key_mask[i])
        ...
        #TODO
        #print("in", h_grid_batched[0,0,:10],  h_key_batched[0,0,:10], z[0,0,0,:10])
        ...
        for trigon,xformK,xformG in zip(self.trigon_key_layers,self.XformKeys,self.XformGrids):
            ...
            #print("mid", h_grid_batched[0,0,:10],  h_key_batched[0,0,:10], z[0,0,0,:10])
