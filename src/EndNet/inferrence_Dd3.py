...
from args_devel import args_based3 as args

import warnings
warnings.filterwarnings("ignore", message="sourceTensor.clone")

NTYPES = 6
ddp = True #turnning this on will automatically shuffle data index
silent = False
...
def load_params(rank):
    ...
    if os.path.exists(modelf):
        ...
        for key in checkpoint["model_state_dict"]:
            ...
            elif key.startswith('se3_Ligand'):
                newkey = key.replace('se3_Ligand.','LigandFeaturizer.')
                trained_dict[newkey] = checkpoint["model_state_dict"][key]
                
            elif key.startswith('se3_Grid'):
                newkey = key.replace('se3_Grid.','GridFeaturizer.')
                trained_dict[newkey] = checkpoint["model_state_dict"][key]
...
def load_data(infile,type='txt',workpath=None,world_size=1,rank=0):
    ...
    if type == 'txt':
        ...
        print(f"loaded {len(tags):d} decoys and split into {len(target_s):d} batches")
        # should modify
...
### train_one_epoch
def train_one_epoch(model,loader,rank,epoch,report_attn):
...
def inferrence(rank,world_size,dumm):
    ...
    with torch.no_grad():
        train_one_epoch(model,data_loader,rank,0,report_attn)
