...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not os.path.exists(self.datadir+'%s.grid.npz'%self.proteins[ip]):
            skip_this = True
            cats = []
        else:
            sample = np.load(self.datadir+'%s.grid.npz'%self.proteins[ip],allow_pickle=True) #motif type & crd only
        ...
        if self.debug:
            print("ngrid/node/edge:", grids.shape[0], G_atm.number_of_nodes(), G_atm.number_of_edges())
    ...
    def make_atm_graphs(self, xyz, grids, obt_fs, bnds, atmnames, CBonly=False, use_l1=False ):
        ...
        w1hot[:,1] = grid_neighs # whether u,v are grid neighbors
