...
MOTIFS = ['None', #0
          'COO','Arg','Amine', # 1~3
          'OH','bb','Amide1', # 4~6
          'Nhis','Ntrp', #7~8 #Ring donor & acceptor
          'Phe', 'SH', #9 #10
          'CH3', 'CH32', 'SCH3', #11~13
          'CHn', 'Acc', 'Don' #14~16 added; Alkyl chain middle, non-ring acceptor, non-ring donor
          ]
SIMPLEMOTIFIDX = [0, 
                  2,4,3, 
                  3,3,3,
                  ...
                  6,6,6,
                  6,2,4,
] #0:none 2:acceptor 3:both 4:donor 6:non-Hbonder

BBMOTIF = (5,'C','O','CA')
POSSIBLE_MOTIFS = \
    {#'CYS':[('SG1','HG','CB'),BBMOTIF],
        'ASP':[(1,'CG','OD1','CB'),BBMOTIF],
        'GLU':[(1,'CD','OE1','CG'),(14,'CG','CB'),BBMOTIF],
        'PHE':[(9,'CG','CD1','CB'),BBMOTIF],
        'HIS':[(7,'ND1','HD1','CG'),(7,'NE2','HE2','CD2'), #donor
               (8,'ND1','CE1','CG'),(8,'NE2','CE1','CD2'), #acceptor
               BBMOTIF], # 
        'LYS':[(3,'NZ','1HZ','CE'),(14,'CD','CG','CB'),BBMOTIF],
        'ASN':[(6,'CG','OD1','CB'),BBMOTIF],
        'SER':[(4,'OG','HG','CB'),BBMOTIF],
        'THR':[(4,'OG1','HG1','CB'),BBMOTIF],
        'GLN':[(6,'CD','OE1','CG'),(14,'CG','CB'),BBMOTIF],
        'TYR':[(9,'CG','CD1','CB'),(4,'OH','HH','CZ'),BBMOTIF],
        'TRP':[(9,'CD2','CE3','CG'),(7,'NE1','HE1','CD1'),BBMOTIF],
        'ARG':[(2,'CZ','NH1','NE'),(14,'CD','CG','CB'),BBMOTIF],
        'ALA':[(11,'CA','CB','N'),BBMOTIF],
        'VAL':[(12,'CB','CG1','CA'),BBMOTIF],
        'ILE':[(11,'CG1','CD1','CB'),(11,'CB','CG2','CA'),BBMOTIF],
        'LEU':[(12,'CG','CD1','CB'),BBMOTIF],
        'MET':[(13,'SD','CE','CG'),(14,'CG','CB'),BBMOTIF],
    }

# return interaction tip-to-base atm
def find_base(aaatm,atms):
    aa,atm = aaatm
    aaatm = aa+atm
    nonbb = atm not in ['N','CA','C','O','H']
    

    b,m = None,None
    if atm == 'H':
        b,m = (atms.index('N'),7) #ring donor
    elif atm == 'O':
        b,m = (atms.index('C'),5)
        
    else:
        if aa not in POSSIBLE_MOTIFS:
            return b,m
        
        SCmotifs = POSSIBLE_MOTIFS[aa][:-1]
        repsmotif = SCmotifs[0]
        if aaatm in ['LYSNZ','SEROG','THROG1','TYROH','HISND1','HISNE2']:
            b = atms.index(atm)
            m = repsmotif[0]
        elif aaatm in ['ASPOD2','GLUOE2','ARGNH2']:
            b = atms.index(repsmotif[1])
            m = repsmotif[0]
        elif aa in ['PHE','ILE','LEU','MET','TRP','TYR','VAL'] and nonbb:
            b = atms.index(repsmotif[1])
            m = repsmotif[0]
        else:
            for motif in SCmotifs:
                if atm == motif[2]:
                    b = atms.index(motif[1])
                    m = motif[0]
