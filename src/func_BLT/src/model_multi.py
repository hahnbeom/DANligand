...
class SE3Transformer(nn.Module):
    ...
    def forward(self, G_bnd, G_atm, G_res, r2a):
        ...
        for i,key in enumerate(self.outtype):
            ...
            b,e = i*nq,(i+1)*nq
            q = torch.transpose(h_atml1,1,2)
            # initial shapes of q is [N_atoms, 3, 2*n_channels]
            ...
            # initial shapes of t is [N_atoms, 3, 2*n_channels]

            for i, layer in enumerate(self.Tblock[b:e]): t = layer(t)
            t = torch.squeeze(t)
            # t.shape is now [N_atoms, 3]
            t = torch.einsum('ij,jk->k', w, t) # collapse out N_atoms dimension
        ...
        dxyz_all = torch.stack(dxyz_all,dim=0).reshape(len(dxyz_all),3)
