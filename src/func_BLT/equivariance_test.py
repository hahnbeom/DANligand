...
def enumerate_an_epoch(model, optimizer, generator,
    ...
    #Rs, offsets = [torch.eye(3), R], [torch.zeros([3]), offset]
    #R = torch.tensor([[0,0,1],[1,0,0],[0,1,0]],dtype=torch.float)
    ...
    for R, offset in zip(Rs, offsets):
        ...
        #print(f"G_bnd.edata['d'][:5]: {G_bnd.edata['d'][:5]}")
        R, offset = R.to(device), offset.to(device)
        if i>2: assert False, "quitting"
        ...
        if False:
            # Debugging to confirm equivariance.
            print(f"\trot: {info['rot'][0]}")
            rot_pred_back = torch.einsum('ijk,kl->ijl',rot_pred, R.to(device))
        ...
        if w_loss[2] > 0 or w_loss[3] > 0:
            ...
            rot_pred = Rotation.from_matrix(rot_pred[motifidx].cpu().detach().numpy()
                    ).as_quat()
