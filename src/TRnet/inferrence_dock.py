...
# efficient version
from src.model_merge_eff import HalfModel
import src.dataset_eff as dataset
...
args.max_subset = 20 #for inferrencing
...
def superimpose_ligand( Ypred, Gligs, keyidx, atms, prefices ):
    i = 0
    rmsd_s = []
    atms_s = []
    for y,n,idx,at,prefix in zip(Ypred,Gligs.batch_num_nodes(),keyidx,atms,prefices):
        # idx: KxN
        x = Gligs.ndata['x'][i:i+n].squeeze()
        idx = torch.where(idx)[1]
        rmsd,atms = myutils.generate_pose(y, idx, x, np.array(at), prefix=prefix)
        rmsd_s.append(rmsd)
        atms_s.append(atms)
        i += n
    return rmsd_s,atms_s

def calc_mae(Yrec, Ylig, nK ):
    N = Yrec.shape[0]
...
def main(prefix):
    ...
    #file check
    missing = []
    for f in ['%s.ligand.mol2'%prefix,'%s.keyatom.def.npz'%prefix,'%s.score.npz'%prefix]:
        if not os.path.exists(f): missing.append(f)
    if missing != []:
        sys.exit('missing files: '+ ' '.join(missing))

    tags = myutils.read_mol2_batch('%s.ligand.mol2'%prefix, tag_only=True)[-1]
    tags.sort() 
    tags_batched = []
    ...
    with torch.no_grad():
        ...
        for i,(G1s,G2s,keyxyz,keyidx,blabel,info) in enumerate(loader):
            ...
            # superpose
            nK = torch.tensor(info['nK']).to(device)
            mae = calc_mae( Yrec, keyxyz, nK )
            rmsd, atms = superimpose_ligand( Yrec, G2s, keyidx, info['atms'], [info['name']] )
            
            print(info['name'], "dev-from-input: ", mae, ", pred-to-key dev: ", rmsd, "keyatoms", atms)
