...
def collate(samples):
    ...
    for s in samples:
        ...
        labelxyz.append(s[2])
        label.append(torch.tensor(s[3])) #no offset; starts from 0 for each graph
        info.append(s[4])
...
def score_loss(Xrec_s, Prec_s, Yrec_s, Xlig_s, keyidx_s, rec_nnodes, lig_nnodes):
    device = Xrec_s.get_device()

    br,bl = 0,0
    B = len(keyidx_s)
    for Yrec,M,N,keyidx in zip(Yrec_s,rec_nnodes, lig_nnodes, keyidx_s):
        Prec = Prec_s[br:br+M]
        Xrec = Xrec_s[br:br+M]
...
for epoch in range(MAXEPOCH):  
    ...
    for i,(G1,G2,labelxyz,keyidx,info,b) in enumerate(train_generator):
        ...
        loss2 = score_loss( Xrec, Prec, Yrec, Xlig, keyidx, G1.batch_num_nodes(), G2.batch_num_nodes() )
