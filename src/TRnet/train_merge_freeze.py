...
# args
from args import args_freeze as args

# Dataset params
K = 4 # keyatoms
datapath = '/home/j2ho/DB/Motifnet'
...
def ranking_loss( pred, label ):
    label_renorm = torch.softmax(label, dim=0)
    pred_renorm = torch.softmax(pred, dim=0)
    loss = torch.sum(label_renorm * torch.log(label_renorm / pred_renorm )) # KL divergence; 
    return torch.sqrt(loss)  # softening
...
def load_data(args_in):
    ...
    target_s = []
    for ln in open('/home/j2ho/vs/MotifScreen/data/0329_decoy.list','r'):
        x = ln.strip().split()
        target = x[0]
        liglist = x
...
def load_model( args_in ):
    ...
    if os.path.exists("models/%s/model.pkl"%modelname): 
        ...
        for name, param in model.named_parameters():
            #if name.startswith('trigon_module') and 'list.4' in name:
            #    print(name, name in found)
            #    param.requires_grad = False
            if name in found:
                param.requires_grad = False
            else:
                pass
                #param.data.fill_( 0.0 ) # zero-initialize
    ...
    with model.no_sync():
        with torch.cuda.amp.autocast(enabled=False):
            for i,(G1s,G2s,keyxyz,keyidx,blabel,info) in enumerate(generator):
                ...
                if train: prefix = "TRAIN-%d %s"%(args_in.rank,info['name'])
                else: prefix = "VALID-%d %s"%(args_in.rank,info['name'])
            
                loss1, mae = structural_loss( prefix, Yrec, keyxyz ) #both are Kx3 coordinates
                loss2 = args_in.w_spread*spread_loss( keyxyz, z, G1s )

                lossAff = args_in.w_class*classification_loss( affinity, blabel )
                
                print(f"{prefix} {i:5d}/{len(generator):5d} {float(loss1):8.3f} ({float(mae):8.3f}) {float(loss2):8.3f} | {float(lossAff):8.3f} :  {' '.join(['%5.2f'%a for a in affinity.tolist()])} : { ' '.join(['%3.1f'%a for a in blabel.tolist()])}" )
