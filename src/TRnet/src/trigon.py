...
class TriangleProteinToCompound_v2(torch.nn.Module):
    ...
    def forward(self, z, protein_pair, compound_pair, z_mask):
        ...
        size_p = protein_pair.shape #(1, i, 1, 1, embedding_channels)
        size_c = compound_pair.shape #(1, j, 1, 1, embedding_channels)

        protein_pair = protein_pair.reshape((size_p[0],size_p[1],size_p[0],size_p[4]))
        compound_pair = compound_pair.reshape((size_c[0],size_c[0],size_c[1],size_c[4]))
