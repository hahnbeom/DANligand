...
class DataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index): #N: maximum nodes in batch
        ...
        try:
            Grec = receptor_graph_from_motifnet(motifnetnpz, self.K,
            ...
            for lig in target_lig_list:
                ...
                keyidx = identify_keyidx(lig, Glig, atms, self.datapath, self.K, self.mixkey)
...
def identify_keyidx(target, Glig, atms, datapath, K, mixkey):
    ...
    if len(keyidx) < K: return None

    if mixkey:
        keyidx = np.random.choice(keyidx,K,replace=False)
    else:
