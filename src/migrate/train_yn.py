#!/usr/bin/env python
from SE3.wrapperR2 import * # sync l1_out_features for rot & WOblock
...
ntypes=len(TYPES)
Ymode = 'node'
n_l1out = 4 # assume important physical properties e.g. dipole, hydrophobicity et al
w_reg = 1e-5
LR = 1.0e-4
...
def l1_loss(truth, w, v, R, cat, Gsize, mode='node_weight'):
    #w: nnode x ntype
    #v: nnode x 3
    s = 0
...
for epoch in range(start_epoch, max_epochs):
    ...
    for G, node, edge, info in train_loader:
        ...
        loss,n  = l1_loss(yaxis, wo, v, R, cat, Gsize, mode=Ymode)
