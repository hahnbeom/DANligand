...
set_params = {
    ...
    #'ballmode'     : 'all',
    #'sasa_method'  : 'sasa',
    #'edgemode'     : 'distT', #default; efficient but contains self edge
    #'edgek'        : (0,0),
    #'edgedist'     : (10.0,6.0), 
    #'distance_feat': 'std',
...
def category_loss(cat, w, cs, Gsize, mode='node'):
    s = 0
    ...
    for i,b in enumerate(Gsize):
        if mode == 'node_weight':
            ...
            #wG = w[s][:,None].repeat(1,2) # ntype x 2 
            q = cs[:,s,:] #type x 2
            #q = cG
        ...
        print("%2d %4.2f %6.3f %6.3f %6.3f %6.3f | "%(cat[i],Qs[cat[i],0],
...
for epoch in range(start_epoch, max_epochs):
    ...
    for G, node, edge, info in train_loader:
        ...
        wo,wb,wc,cs,v,R = model(to_cuda(G, device), to_cuda(node, device), to_cuda(edge, device))
        ...
        loss1,loss2 = category_loss(cat, wc, cs, Gsize)
        ...
        temp_loss["grp"].append(loss2.cpu().detach().numpy()) #store as per-sample loss
    ...
    with torch.no_grad(): # wihout tracking gradients
        ...
        for k in range(3): #repeat multiple times for stability
            for G, node, edge, info in valid_loader:
                ...
                temp_loss["grp"].append(loss2.cpu().detach().numpy())
