...
class SE3Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        # Atom
        l1_out_channels = self.num_degrees*self.num_channels[2] 
        ...
        for key in self.outtype:
            ...
            Wblock.append(nn.ReLU(inplace=True)) #always positive value

            # For now just use a single linear layer for the Qblock and Tblock.
            # > 1 breaks?
            # Nonlinearities are more delicate for order l1 representations
            # l-1: GMM backbone coord
            Tblock.append(nn.Linear(self.num_degrees*self.num_channels[2],
                                    self.nGMM, # each channel goes to each GMM
                                    bias=False))

            # outer dim should be 2 to predict two equivariant directions.
            if self.learn_OR == 'split': 
                Qblock.append(nn.Linear(self.num_degrees*self.num_channels[2],
                                        self.num_degrees*self.num_channels[2], bias=False))
                Qblock.append(nn.Linear(self.num_degrees*self.num_channels[2], 1, bias=False))
                Ablock.append(nn.Linear(self.num_degrees*self.num_channels[2], 1, bias=False))
                
            elif self.learn_OR in ['gs','axis']: #GMM route: 'axis'
                # y-axis prediction
                '''
                Qblock.append(nn.Linear(self.num_degrees*self.num_channels[2],
                                        self.num_degrees*self.num_channels[2],bias=False))
            ...
            Gblock.append(nn.Linear(self.num_degrees*self.num_channels[2],
                                    self.nGMM*2 # :nGMM to sigma, nGMM: to probability
                                    ))
            Gblock.append(nn.ReLU(inplace=True)) # make always positive

        self.Cblock = nn.ModuleList(Cblock)
        self.Wblock = nn.ModuleList(Wblock)

        self.Qblock = nn.ModuleList(Qblock) #orientation: Y-axis
        self.Tblock = nn.ModuleList(Tblock) # BB coord
        #self.Ablock = nn.ModuleList(Ablock) # unused
        self.Gblock = nn.ModuleList(Gblock) # Amplitude & Sigma
    ...
    def forward(self, G_bnd, G_atm, G_res, r2a):
        ...
        def runlayer(layer, G, r, basis):
            def custom_forward(*h):
                ...
                #if "1" in hd.keys():
                    #print(f"\t\thd['1'].shape:{hd['1'].shape}")
                    #print(f"\t\thd['1'][0,0]:{hd['1'][0,0]}")
        ...
        nt = int(len(self.Tblock)/len(self.outtype))
        ng = int(len(self.Gblock)/len(self.outtype))

        # works only for single batch!
        n = h_atml0.shape[0] # functype
        ...
        # Iterate over predictions of translations and rotations for each
        # functional group (14 groups total)

        # In the block below, the variables nc, nw, nq and nt are the number of
        # layers t be applied for each functional group.
        for i,key in enumerate(self.outtype):
            ...
            b,e = i*nw,(i+1)*nw
            w = h_atml0
            for j, layer in enumerate(self.Wblock[b:e]):
                w = layer(w)
            ...
            q = torch.transpose(h_atml1,1,2) #put channel at the last
            # initial shapes of q is [N_atoms, channels, 3]
            for layer in self.Qblock[b:e]: q = layer(q)

            # sharing the same weight tensor "w" okay?
            # q.shape is now [N_atoms, 3, 2]
            if self.learn_OR == 'gs':
                q = torch.einsum('ij,jkl->kl', w, q) # collapse out N_atoms dimension
                  # q.shape is now [3, 2]
                q = myutils.GramSchmidt(q)
                
            elif self.learn_OR == 'axis':
                # simple normalization
                q = torch.mean(q,axis=0).squeeze()
                q = q/torch.sqrt(torch.sum(q*q)) #normalize
                
            elif self.learn_OR == 'split':
                #q = torch.einsum('ij,jkl->kl', w, q).squeeze() # collapse out N_atoms dimension
                q = torch.mean(q,axis=0).squeeze()
                q = q/torch.sqrt(torch.sum(q*q)) #normalize
                # reduce channel to 1 -> mean over nodes
                ang = torch.mean(self.Ablock[i](h_atml0),axis=0)
                ang = 3.141592*(2.0*ang-1.0) #normalize  around -pi~pi
                
                q = q*torch.cos(ang/2) #differentiable?
                q = torch.cat((torch.sin(ang/2),q),axis=0)

            # GMM-learning
            b,e = i*nt,(i+1)*nt
            t = torch.transpose(h_atml1,1,2)
            # initial shapes of t is [N_atoms, 3, nchannel]
            for i, layer in enumerate(self.Tblock[b:e]): t = layer(t)
            
            t = torch.squeeze(t)
            # t.shape is now [natm,nGMM,3]
            t = torch.einsum('ij,jkl->kl', w, t) # collapse out N_atoms dimension

            # GMM params (l0); sig, Ampl
            b,e = i*ng,(i+1)*ng
            g = h_atml0
            for j, layer in enumerate(self.Gblock[b:e]):
                g = layer(g)
            g = torch.einsum('ij,jk->k', w, g) # collapse out N_atoms dimension
            
            sigs =  4.5*torch.tanh(g[:3])+0.5
            
            # convert to reasonable range (positive value to )
            ampls = g[3:]
            # make sure normalized correctly
            ampls = ampls/torch.sqrt(torch.sum(ampls*ampls))
            # then divide by sigma normalizer to make it as "normal dstr"
            ampls = ampls / (sigs*np.sqrt(2*np.pi)+1.0e-6)

            logits_all.append(logit)
            dxyz_all.append(t)
            ampl_all.append(ampls)
        ...
        dxyz_all = torch.stack(dxyz_all,dim=0).reshape(len(dxyz_all),self.nGMM,3)
        orientations_all = torch.stack(orientations_all,dim=0)
        sigs_all = torch.stack(sigs_all,dim=0)
        ampl_all = torch.stack(ampl_all,dim=0)
        
        return Pall, dxyz_all, orientations_all, ampl_all, sigs_all
