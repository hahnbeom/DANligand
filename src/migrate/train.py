...
def l1_loss(truth, w, v, R, cat, Gsize):
    #w: nnode x ntype
    #v: nnode x 3
    s = 0
...
for epoch in range(start_epoch, max_epochs):
    ...
    for G, node, edge, info in train_loader:
        if G != None:
            ...
            temp_loss["total"].append(loss.cpu().detach().numpy()/n) #store as per-sample loss
            ...
            if (b_count+1)%accum == 0:
                ...
                print("TRAIN Epoch(%s): [%2d/%2d], Batch: [%2d/%2d], loss: %.3f, error %d"
                       %(modelname, epoch, max_epochs, b_count, len(train_loader), np.sum(temp_loss["total"][-1*accum:])/n,e_count)) 
