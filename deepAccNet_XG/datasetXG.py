...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(len(aas_lig),len(aas_rec),len(aas),len(xyz_lig),len(xyz_rec))
...
def load_dataset(set_params, generator_params, setsuffix='v5', load_VSset=False,
    ...
    if load_VSset:
        cross_set_train = Dataset(np.load("data/trainVS_proteins%s.npy"%setsuffix), **set_params)
        cross_set_valid = Dataset(np.load("data/validVS_proteins%s.npy"%setsuffix), **set_params)
