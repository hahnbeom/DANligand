...
class SE3Transformer(nn.Module):
    ...
    def __init__(self, 
        ...
        # weight factor per atm
        if self.dGlayer != 'old':
            Ublock = []
            Ublock.append(nn.Linear(self.num_degrees*self.num_channels[2],
                                    self.num_degrees*self.num_channels[2]))
            Ublock.append(nn.Dropout(drop_out)) 
            Ublock.append(nn.ReLU(inplace=True))
            Ublock.append(nn.Linear(self.num_degrees*self.num_channels[2], 1))
            Ublock.append(nn.Sigmoid())
            self.Ublock = nn.ModuleList(Ublock)
    ...
    def forward(self, G_bnd, G_atm, G_res, idx):
       ...
       ## Energy path for binding affinity
        ...
        E = h_atm
        for layer in self.Eblock: E = layer(E) #feature-space pooled at the last: Nx1

        if self.dGlayer != 'old':
            u = h_atm #weight vector over entire atom-graph as sigmoid (0~1)
            for layer in self.Ublock: u = layer(u)

            if self.dGlayer == 'ligandonly':
                u = torch.matmul(idx,u) #1xN
                E = torch.matmul(idx,E.squeeze(1)) #Nxbins
            #elif self.dGlayer == 'full': #multiply all
            
            u = torch.transpose(u,0,1)
            E = torch.matmul(u,E) # out: nbins
        ...
        # is this pool over atom?
        dG  = torch.sum(E,axis=0) # dimension: [1,8]
